...
def podaciSpremi_xml(lista_ucenika,PodaciUčenikaXML):
    root = ET.Element("Učenici")
    for p in lista_ucenika:
        učenik_el = ET.SubElement(root, "Učenik")
        ET.SubElement(učenik_el, "ime").text = p.ime
        ET.SubElement(učenik_el, "prezime").text = p.prezime
        ET.SubElement(učenik_el, "razred").text = p.razred
    
    xml_string = ET.tostring(root, 'utf-8')
    dom = minidom.parseString(xml_string)
    lijepi_xml_string = dom.toprettyxml(indent="  ")
    with open(PodaciUčenikaXML, "w", encoding='utf-8') as f:
        f.write(lijepi_xml_string)

    print(f'Spremljeno u {PodaciUčenikaXML}')

def podaciUčitaj_xml(PodaciUčenikaXML):
    
    tree = ET.parse(PodaciUčenikaXML)
    root = tree.getroot()
    for učenik_el in root.findall("Učenik"):
        ime = učenik_el.find("ime").text
        prezime = učenik_el.find("prezime").text
        razred = učenik_el.find("razred").text
        lista_ucenika.append(Ucenik(ime, prezime, razred))

    print(f"Učitano iz {PodaciUčenikaXML}")
...
while True:
    ...
    try:
        ...
        if izbor == 1:
            ...
            podaciSpremi_xml(lista_ucenika, 'PodaciUčenikaXML')

        elif izbor == 5:
            podaciUčitaj_xml('PodaciUčenikaXML')
        ...
        print ("Molimo unesite ispravan odabir (0/1/2/3/4/5).")
